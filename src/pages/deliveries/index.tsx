...
import { useRoqClient, useDeliveryFindManyWithCount } from 'lib/roq';
...
export function DeliveryListPage(props: DeliveryListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['status.contains', 'address.contains', 'contact_number.contains'],
          ...params.filters,
        },
        'delivery',
      ),
  ...
  const { data, error, isLoading, mutate } = useDeliveryFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.delivery.delete({ where: { id } });
      await mutate();
    } catch (error) {
