...
import { useRoqClient, useDeliveryFindFirst } from 'lib/roq';
...
function DeliveryEditPage() {
  ...
  const { data, error, isLoading, mutate } = useDeliveryFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: DeliveryInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.delivery.update({
        data: values as RoqTypes.delivery,
        where: {
          id,
        },
      });
